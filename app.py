...
.phone{width:380px;min-height:850px;background:linear-gradient(180deg,#2e40e6 0%, #1a2480 40%, #050516 100%);border-radius:35px;padding:18px;box-shadow:0 0 0 8px #e5e5e5, 0 20px 60px rgba(0,0,0,0.6);position:relative;box-sizing:border-box;color:white}
.top-bar{display:flex;justify-content:space-between;align-items:center;padding:10px 5px}
.icon-btn{width:36px;height:36px;background:rgba(0,0,0,0.3);border-radius:50%;display:flex;justify-content:center;align-items:center;color:white;font-size:14px;border:none;cursor:pointer;text-decoration:none}
...
.chat-pill{background:#2a2fb8;border-radius:20px;padding:12px 16px;margin:8px auto;width:90%;font-size:13px}
.chat-pill img.post-media{width:100%;border-radius:15px;margin-top:8px;max-height:250px;object-fit:cover}
.chat-pill video.post-media{width:100%;border-radius:15px;margin-top:8px;max-height:300px}
.comment-box{background:rgba(0,0,0,0.25);border-radius:12px;padding:6px 10px;margin-top:6px;font-size:11px}
.comment-input{width:65%;background:rgba(255,255,255,0.1);border:1px dashed rgba(255,255,255,0.2);border-radius:20px;padding:6px 10px;color:white;outline:none;font-size:11px}
.search-box{width:92%;margin:8px auto;display:flex;gap:5px}
...
.post-form{background:rgba(0,0,0,0.2);border-radius:20px;padding:10px;margin:10px auto;width:92%;box-sizing:border-box}
...
def home():
    ...
    if request.method=='POST':
        ...
        if media_file and media_file.filename:
            ...
            if ext in ['mp4','mov','avi','mkv','webm']: mtype='video'
            else: mtype='photo'
    ...
    <div class='top-bar'><a class='icon-btn' href='#' onclick="document.getElementById('searchArea').style.display='flex';return false">🔍</a><div style='font-weight:bold'>@{user}</div><div style='display:flex;gap:5px'>{admin_link}<button class='icon-btn' onclick="document.getElementById('body').classList.toggle('light')">🌙</button><a class='icon-btn' href='/logout'>⎋</a></div></div>
    <div id='searchArea' class='search-box' style='display:{"flex" if q else "none"}'><form method='GET' style='display:flex;gap:5px;width:100%'><input class='search-inp' name='q' value='{q}' placeholder='Search users or posts...'><button class='icon-btn' style='width:45px'>Go</button><a class='icon-btn' href='/'>✕</a></form></div>
    ...
    <form class='post-form' method='POST' enctype='multipart/form-data'><div style='display:flex;gap:5px'><input name='content' placeholder='Type a message...' style='flex:1;background:rgba(80,80,130,0.5);border:2px dashed rgba(255,255,255,0.15);border-radius:30px;padding:10px 15px;color:white;outline:none'><button style='background:#5a7cff;border:none;border-radius:50%;width:42px;height:42px;color:white'>↑</button></div><div style='margin-top:8px;display:flex;gap:8px;align-items:center'><label style='background:rgba(255,255,255,0.15);padding:6px 12px;border-radius:20px;font-size:11px;cursor:pointer'>📷 Photo/Video<input type='file' name='media' accept='image/*,video/*' hidden></label><span style='font-size:10px;opacity:0.6'>Upload with post</span></div></form>
...
def admin():
    user=session.get('user')
    if user!=ADMIN_USER: return "<h1>⛔ Not Admin! Only Sayan can access</h1><a href='/'>Back</a>"
    conn=sqlite3.connect('users.db'); cur=conn.cursor()
    ...
    conn.close()
    users_html="".join([f"<div class='admin-card'><b>@{u}</b> • {e} {'📷' if ph else '👤'} <a href='/admin_del_user/{u}' style='color:#ff6b6b;float:right;text-decoration:none'>Delete User</a></div>" for u,e,ph in users])
    posts_html="".join([f"<div class='admin-card'><b>#{pid} @{un}</b> • {tm} <br>{ct[:50]} {'📷' if md else ''} <a href='/delete/{pid}' style='color:#ff6b6b;float:right'>✕ Delete</a></div>" for pid,un,ct,tm,md in posts[:20]])
    return f"<html><head><meta name='viewport' content='width=device-width, initial-scale=1'><style>{CSS}</style></head><body><div class='phone'><div class='top-bar'><a class='icon-btn' href='/'>←</a><b>👑 ADMIN PANEL</b><div class='icon-btn'>@{user}</div></div><div style='background:linear-gradient(90deg,#ffaa00,#ff6a00);padding:15px;border-radius:20px;margin:10px 0;text-align:center'><div style='font-size:28px;font-weight:800'>{total_posts} Posts</div><div style='font-size:14px'>{len(users)} Users • {total_likes} Likes</div></div><h3 style='font-size:12px'>USERS ({len(users)})</h3><div style='max-height:200px;overflow:auto'>{users_html}</div><h3 style='font-size:12px;margin-top:15px'>RECENT POSTS</h3><div style='max-height:300px;overflow:auto'>{posts_html}</div></div></body></html>"
...
def login():
    ...
    return f"<html><head><meta name='viewport' content='width=device-width, initial-scale=1'><style>{CSS}</style></head><body><div class='phone'><h2 style='text-align:center;margin-top:80px;font-size:32px'>Login</h2><p style='text-align:center;opacity:0.7;font-size:12px'>Sign in to continue.</p><form method='POST'><input style='width:100%;background:rgba(80,80,130,0.5);border:2px dashed rgba(255,255,255,0.15);border-radius:30px;padding:14px 20px;margin-bottom:12px;color:white;box-sizing:border-box' name='username' placeholder='NAME' required><input style='width:100%;background:rgba(80,80,130,0.5);border:2px dashed rgba(255,255,255,0.15);border-radius:30px;padding:14px 20px;margin-bottom:12px;color:white;box-sizing:border-box' name='password' type='password' placeholder='PASSWORD' required><button style='width:100%;background:linear-gradient(90deg,#6a8cff,#4a5cff);border:none;padding:14px;border-radius:10px;font-weight:800;cursor:pointer;color:#0a0a2a;margin-top:15px'>Login</button></form><a href='/signup' style='display:block;text-align:center;margin-top:15px;color:#00aaff;text-decoration:none;font-size:12px'>Signup!</a></div></body></html>"
@app.route('/logout')
def logout():
