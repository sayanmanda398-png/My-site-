...
def home():
 ...
 cur.execute('SELECT bio FROM users WHERE username=?',(user,));r=cur.fetchone();bio=r[0] if r and r[0] else "No bio"
 ...
 conn.close()
 return f"<html><head><meta name='viewport' content='width=device-width, initial-scale=1'><style>{CSS}</style></head><body><div class='phone'><b>@{user}</b> | {followers} Followers | {following} Following | {bio} <a href='/edit_profile' class='btn-follow'>Edit Bio</a><form method='POST'><input name='content' placeholder='Post...' style='width:70%;padding:10px;border-radius:20px;color:black'><button style='background:#5a7cff;border-radius:50%;width:38px;height:38px'>↑</button></form>{html}</div></body></html>"
@app.route('/follow/<u>')
...
def edit_profile():
 ...
 return f"<html><body><div style='max-width:360px;margin:auto;background:#222;color:white;padding:20px;border-radius:20px'><a href='/'>Back</a><h3>Edit Bio</h3><form method='POST'><textarea name='bio' maxlength='150' style='width:100%;height:80px;color:black'>{bio}</textarea><button>Save</button></form></div></body></html>"
...
def signup():
 ...
 return "<html><body><h2>Signup</h2><form method='POST'><input name='username' placeholder='NAME'><input name='email'><input name='password' type='password'><button>Sign up</button></form></body></html>"
