...
def init_db():
    ...
    conn.execute('CREATE TABLE IF NOT EXISTS posts (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, content TEXT, time TEXT)')
...
def home():
    ...
    if request.method=='POST' and user:
        ...
        if content:
            ...
            cur.execute('INSERT INTO posts (username,content,time) VALUES (?,?,?)',(user,content,now))
            conn.commit()
    cur.execute('SELECT username,content,time FROM posts ORDER BY id DESC')
    posts=cur.fetchall()
    conn.close()
    ...
    if not user:
        ...
        .card{background:white;color:#0f172a;padding:30px;border-radius:20px;max-width:380px;margin:20px auto}
        ...
        </style></head><body><h1>🚀 BABU PRO</h1><p>Day 8 - Real Posts</p>
        <div class="card"><h2>Welcome Guest!</h2><a class="btn" href="/login">Login</a><a class="btn btn2" href="/signup">Signup</a></div></body></html>"""

    posts_html=""
    for u,c,t in posts:
        posts_html+=f"<div class='post'><b>@{u}</b><p>{c}</p><p style='color:gray'>{t} • ❤️ likes</p></div>"
    if not posts_html:
        posts_html="<div class='post'><p>No posts yet! Be first!</p></div>"

    return f"""
    <html><head><meta name="viewport" content="width=device-width, initial-scale=1"><style>
    body{{font-family:sans-serif;background:#f1f5f9;margin:0}}
    .nav{{background:#0f172a;color:white;padding:15px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0}}
    ...
    .create{{background:white;border-radius:20px;padding:15px;margin-bottom:15px}}
    textarea{{width:95%;padding:10px;border-radius:10px;border:1px solid #ccc;resize:none}}
    button{{background:#0f172a;color:white;padding:10px 20px;border-radius:10px;border:0;margin-top:8px;width:100%}}
    </style></head><body>
    <div class="nav"><b>BABU PRO - Day 8</b><span>@{user} <a href="/logout">Logout</a></span></div>
    <div class="feed">
    <div class="create"><form method="POST"><textarea name="content" placeholder="What's on your mind, {user}?" required rows="3"></textarea><button>Post</button></form></div>
    {posts_html}
    </div></body></html>
    """
...
def login():
    if request.method=='POST':
        ...
        if ok:
            ...
            return "<h2 style='text-align:center;color:red'>Wrong!</h2><a href='/login' style='display:block;text-align:center'>Try Again</a>"
